...
lol.anim=
  ...
  update:function()
    ...
    for(var i=0;i<n;i++)
      {
      var test=true,x2=star[i].px,y2=star[i].py;
      star[i].x+=lol.mouse.p.x*0.1;
      if(star[i].x>lol.x*3){star[i].x-=lol.w*3;test=false;}
      if(star[i].x<-lol.x*3){star[i].x+=lol.w*3;test=false;}
      star[i].y+=lol.mouse.p.y*0.1;
      if(star[i].y>lol.y*3){star[i].y-=lol.h*3;test=false;}
      if(star[i].y<-lol.y*3){star[i].y+=lol.h*3;test=false;}
      star[i].z-=lol.spd;
      if(star[i].z>lol.z){star[i].z-=lol.z;test=false;}
      if(star[i].z<0){star[i].z+=lol.z;test=false;}
  ...
  pause:function(){lol.anim[lol.rid?'stop':'start']();}
  };
lol.mouse=
  {
  p:{x:0,y:0}, /* position */
...
lol.key=function(e)
  ...
  switch(e.keyCode)
    {
    case 13:lol.anim.pause();break;
    case 32:hyperspace=1;break;
    }
  };
